...
```{r}
wa_sc <- data.frame(scores(redun_bal, scaling = 2, display = "wa"))
sp_sc <-  data.frame(scores(redun_bal, scaling = 2, display = "species"), variable=ilrDef, type='sp')
bp_sc <- scores(redun_bal, scaling = 2, display = "bp")
...
ggplot(wa_sc, aes(x=RDA1, y=RDA2)) +
  ...
  geom_point(alpha=0.07, size=4) +
  #geom_segment(data=sp_bp_sc, aes(x=0, y=0, xend=RDA1, yend=RDA2), size=0.5,
  #             arrow=arrow(length=unit(0.2,"cm"))) +
  geom_point(data = sp_bp_sc, mapping = aes(colour=type, fill=type), size=2) +
  geom_label_repel(data = sp_bp_sc, mapping = aes(colour=type, fill=type, label=variable)) +
  scale_fill_manual(values = c('black', 'white')) +
  scale_colour_manual(values = c('white', 'black')) +
  theme_bw() +
  theme(legend.position = 'none')
ggsave(filename = "images/RDA.png", width = 8, height = 8, dpi=300)
```
...
ggsave(filename = "images/lda_detrended_sensibility.png", width = 10, height = 5, dpi=300)
```
